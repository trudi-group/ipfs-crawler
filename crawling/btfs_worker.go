...
var ProtocolBTFSStrings []protocol.ID = []protocol.ID{
	"btfs/0.1.0",
}
...
func configureBTFSWorker() CrawlerConfig {
...
func NewBTFSWorker(id int, ctx context.Context) *BTFSWorker {
	// ToDo: Not sure if we should 1) derive a new context 2) store the context
	config := configureBTFSWorker()
    ...
    // mnemonic := "relax master language motor slogan abuse lizard forward replace blood pole quiz"
    // keytype := "BIP39"
...
func (w *BTFSWorker) CrawlPeer(askPeer *peer.AddrInfo) (*NodeKnows, error) {
	...
	time.Sleep(time.Duration(mrand.Intn(w.config.MaxBackOffTime)) * time.Millisecond)
	...
	dhtStream, err := w.h.NewStream(ctx, recvPeer.ID, ProtocolBTFSStrings...)
	if err != nil {
...
func (w *BTFSWorker) AddPreimages(handler *PreImageHandler)  {
	w.ph = handler
}
func (w *BTFSWorker) Capacity () int {
	return w.capacity
}
...
func (w *BTFSWorker) FullNeighborCrawl(ctx context.Context, remotePeerStream network.Stream,
