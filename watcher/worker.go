...
const protocolBitswapOneOne protocol.ID = "/ipfs/bitswap/1.1.0"
const protocolBitswapOneZero protocol.ID = "/ipfs/bitswap/1.0.0"
const protocolBitswapNoVers protocol.ID = "/ipfs/bitswap"
    ...
    for {
        select {
            ...
            case event := <- channel:
                ...
                for _,cid := range event.Haves {
                    cidMap[cid] = true
                }
                for _,cid := range event.DontHaves {
                    cidMap[cid] = true
                }
...
func (w* BSWorker) queryContent(stream network.Stream, cids []cid.Cid) error{
    ...
    for _, cid := range cids {
        wl.AddEntry(cid, 1, pb.Message_Wantlist_Have, true)
    }
    log.WithFields(log.Fields{
        "Remote": stream.Conn().RemotePeer(),
        "request": wl.Loggable(),
